...
import { CATEGORIES_QUERY } from "@/grqphql/schema";
...
const Navbar = () => {
  ...
  const { categories, cart, initializeCategories, cartToggle, toggleCart } =
  ...
  useEffect(() => {
    if (!loading && !error && data?.categories) {
      if (
        location.pathname === "/" ||
        !data.categories.some((route: ICategory) =>
  ...
  return (
    <nav className="sticky top-0 left-0 w-full h-[5rem] z-[200] bg-white py-6 px-[6.25rem] grid grid-cols-3 justify-center items-center">
      <ul className="flex items-center col-span-1 h-full w-full ">
        {categories.map((category) => (
          <li key={category.name}>
            <Link
              ...
              className={`h-[3.5rem]  w-[6.25rem] px-4 flex items-center ${
                location.pathname.includes(category.name) &&
                "border-b-2 border-[#5ECE7B]"
              } font-semibold uppercase text-base ${
                location.pathname.includes(category.name) && "text-[#5ECE7B]"
              }  justify-center `}
            >
              {category.name}
            </Link>
        ...
        >
          {cart.itemsQuantity > 0 && (
            <div
              ...
              className="absolute bg-black w-6 h-6 rounded-full -right-4 -top-3 text-white font-semibold"
            >
              {cart.itemsQuantity}
            </div>
          )}
